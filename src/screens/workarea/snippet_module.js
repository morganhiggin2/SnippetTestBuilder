...
export function generateSnippet(
  ...
  for (var i = 0; i < pipeline_connectors.length; i++) {
    if (pipeline_connectors[i].input) {
      leftPipeInserts.push(
        createPipeInsert(
          pipeline_connectors[i].id,
          visualComponents,
          pipeline_connectors[i].name,
          xPos,
          yPos + textHeight + 8,
          true,
          spawnPipeline,
  ...
  var leftPipeInsertsWidth = Math.max.apply(
    Math,
    leftPipeInserts.map((pipeInsert) => pipeInsert.width),
  );
  var rightPipeInsertsWidth = Math.max.apply(
    Math,
    rightPipeInserts.map((pipeInsert) => pipeInsert.width),
  );
  var pipeInsertsHeight = Math.max(
    leftPipeInserts
      .map((pipeInsert) => pipeInsert.height)
      .reduce((partSum, a) => partSum + a + 4, 0),
    rightPipeInserts
      .map((pipeInsert) => pipeInsert.height)
      .reduce((partSum, a) => partSum + a + 4, 0),
  );
  ...
  let rectWidth = Math.max(
    textWidth + 40,
    leftPipeInsertsWidth + 4 + rightPipeInsertsWidth,
    20,
  );
  ...
  titleText.setPosition({
    x: xPos + (rectWidth - textWidth) / 2,
    y: titleText.getPosition().y,
  });
  ...
  //snippet events
  titleBackgroundRect.on("dblclick", () => {
    deleteSnippet(id);
  });
  snippet_group.on("dragstart", () => {
    dragStart(id);
  });
...
export function generatePipeConnector(
  id,
  visualComponents,
  x_pos_start,
  y_pos_start,
...
export function setNewPositionPipeConnector(
  pipeConnector,
  x_pos_start,
  y_pos_start,
  x_end_offset,
...
function createPipeInsert(
  ...
  if (left) {
    titleText.setPosition({
      x: xPos + 10,
      y: yPos + (14 - titleText.getHeight()) / 2,
    });
  } else {
    titleText.setPosition({
      x: xPos - titleTextWidth - 10,
      y: yPos + (14 - titleText.getHeight()) / 2,
    });
  }
  ...
  backgroundRect.on("mouseover", () => {
    backgroundRect.fill(visualComponents[id].state.highlight_color);
  });
  backgroundRect.on("mouseout", () => {
    backgroundRect.fill(visualComponents[id].state.color);
  });
  backgroundRect.on("click", () => {
    spawnPipeline(id, pipelineConnectorPositionOffset);
  });
