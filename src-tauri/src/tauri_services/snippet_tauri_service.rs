...
use crate::{
    core_services::concurrent_processes::spawn_run_snippets_event,
    python_libraries::python_run_module::InitializedPythonSnippetRunnerBuilder,
    state_management::{
        visual_snippet_component_manager::{FrontPipelineContent, FrontSnippetContent},
...
pub fn new_snippet(
    ...
    let directory_uuid =
        match visual_directory_component_manager.find_directory_entry_uuid(&directory_front_uuid) {
            Some(result) => result,
            None => {
    ...
    let external_snippet =
        match external_snippet_manager.find_external_snippet_from_directory_uuid(directory_uuid) {
            Some(result) => result,
            None => {
    ...
    let front_snippet = snippet.get_snippet_to_front_snippet(
...
#[tauri::command]
pub fn get_pipeline_connector_uuids_from_snippet(
    application_state: tauri::State<SharedApplicationState>,
    window_session_uuid: Uuid,
    front_uuid: Uuid,
) -> Result<Vec<Uuid>, &str> {
    ...
    let mut front_pipeline_connector_uuids: Vec<Uuid> =
        Vec::with_capacity(pipeline_connector_uuids.len());

    for pipeline_connector_uuid in pipeline_connector_uuids {
        let front_pipeline_connector_uuid = match visual_snippet_component_manager
            .find_pipeline_connector_front_uuid(&pipeline_connector_uuid)
        {
            Some(result) => result,
            None => {
...
#[tauri::command]
pub fn update_snippet_parameter_value(
    application_state: tauri::State<SharedApplicationState>,
    window_session_uuid: Uuid,
    front_uuid: Uuid,
    value: String,
    ...
    let parameter_uuid = match visual_snippet_component_manager
        .find_parameter_uuid_from_parameter_front(front_uuid)
    {
        Some(some) => some,
        None => {
...
pub fn delete_snippet(
    ...
    for pipeline_connector_uuid in pipeline_connector_uuids.iter() {
        ...
        match snippet_manager.find_pipeline(&pipeline_connector_uuid) {
            Some(_) => {
                return Err(
                    "cannot delete snippet becuase pipelines are still connected to this snippet",
                );
            }
            None => {
                continue;
    ...
    for pipeline_connector_uuid in pipeline_connector_uuids.iter() {
        match visual_snippet_component_manager
            .delete_pipeline_connector_by_internal(&pipeline_connector_uuid)
        {
            Ok(_) => (),
            Err(err) => {
                return Err(err);
...
pub fn new_pipeline(
    application_state: tauri::State<SharedApplicationState>,
    window_session_uuid: Uuid,
    from_front_uuid: Uuid,
    to_front_uuid: Uuid,
) -> Result<FrontPipelineContent, &str> {
    // get the state
    let state_guard = &mut application_state.0.lock().unwrap();
    let state = state_guard.deref_mut();
    ...
    let from_uuid =
        match visual_snippet_component_manager.find_pipeline_connector_uuid(&from_front_uuid) {
            Some(result) => result,
            None => {
                return Err(
                "could not find from pipeline connector uuids from front pipeline connector uuid",
            );
            }
        };

    let to_uuid =
        match visual_snippet_component_manager.find_pipeline_connector_uuid(&to_front_uuid) {
            Some(result) => result,
            None => {
                return Err(
                    "could not find to pipeline connector uuids to front pipeline connector uuid",
                );
            }
        };
    ...
    let pipeline_uuid: Uuid =
        match snippet_manager.create_pipeline(&mut sequential_id_generator, from_uuid, to_uuid) {
            Ok(result) => result,
            Err(err) => {
                return Err(err);
            }
    ...
    let pipeline_front = pipeline.create_pipeline_as_front_content(
        visual_snippet_component_manager,
        sequential_id_generator,
    );
...
#[tauri::command]
pub fn validate_pipeline_connection(
    application_state: tauri::State<SharedApplicationState>,
    window_session_uuid: Uuid,
    from_front_uuid: Uuid,
    to_front_uuid: Uuid,
...
#[tauri::command]
pub fn get_snippet_pipelines(
    application_state: tauri::State<SharedApplicationState>,
    window_session_uuid: Uuid,
    snippet_front_uuid: Uuid,
) -> Result<Vec<Uuid>, &str> {
    // get the state
    let state_guard = &mut application_state.0.lock().unwrap();
    let state = state_guard.deref_mut();
    ...
    for pipeline_connector_uuid in pipeline_connector_uuids {
        ...
        let pipeline_component_uuids =
            snippet_manager.find_pipeline_uuids_from_pipeline_connector(&pipeline_connector_uuid);

        // for each
        for pipeline_component_uuid in pipeline_component_uuids.iter() {
            //get pipeline front uuid
            let pipeline_front_uuid = match visual_snippet_component_manager
                .find_pipeline_front_uuid(&pipeline_component_uuid)
            {
                Some(result) => result,
                None => {
...
#[tauri::command]
pub fn check_pipeline_connector_capacity_full(
    application_state: tauri::State<SharedApplicationState>,
    window_session_uuid: Uuid,
    front_pipeline_connector_uuid: Uuid,
) -> Result<bool, &str> {
    // get the state
    let state_guard = &mut application_state.0.lock().unwrap();
    let state = state_guard.deref_mut();
    ...
    let pipeline_connector_uuid = match visual_snippet_component_manager
        .find_pipeline_connector_uuid(&front_pipeline_connector_uuid)
    {
        Some(result) => result,
        None => {
            return Err(
                "could not find pipeline connector uuid from front pipeline connector uuid",
            );
        }
    };
...
/// get front pipeline connector uuids for front pipeline uuid
#[tauri::command]
pub fn get_pipeline_connector_uuids_from_pipeline(
    application_state: tauri::State<SharedApplicationState>,
    window_session_uuid: Uuid,
    front_pipeline_uuid: Uuid,
) -> Result<FrontPipelineConnectorResult, &str> {
    // get the state
    let state_guard = &mut application_state.0.lock().unwrap();
    let state = state_guard.deref_mut();
    ...
    //get pipeline uuid from front uuid
    let pipeline_uuid =
        match visual_snippet_component_manager.find_pipeline_uuid(&front_pipeline_uuid) {
            Some(result) => result,
            None => {
                return Err("could not find pipeline uuid from front pipeline uuid");
            }
    ...
    let front_from_pipeline_connector_uuid = match visual_snippet_component_manager
        .find_pipeline_connector_front_uuid(&pipeline_component.get_from_pipeline_connector_uuid())
    {
        Some(result) => result,
        None => {
            return Err(
                "could not find front from pipeline connector uuid from pipeline component",
            );
        }
    };

    //get front to pipeline connector uuid
    let front_to_pipeline_connector_uuid = match visual_snippet_component_manager
        .find_pipeline_connector_front_uuid(&pipeline_component.get_to_pipeline_connector_uuid())
    {
        Some(result) => result,
        None => {
...
pub fn spawn_run_snippets(
    ...
    let directory_manager = &mut state.directory_manager;
    //find window session
    let window_session = match state
        .window_manager
        .find_window_session_mut(window_session_uuid)
    {
        Some(result) => result,
    ...
    let build_state = match InitializedPythonSnippetRunnerBuilder::build(
        snippet_manager,
