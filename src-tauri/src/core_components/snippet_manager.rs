use crate::state_management::visual_snippet_component_manager::{
    FrontPipelineConnectorContent, FrontPipelineContent, FrontSnippetContent,
};
use crate::utils::sequential_id_generator::Uuid;
use crate::{
    state_management::{
        external_snippet_manager::{ExternalSnippet, ExternalSnippetParameterType},
        visual_snippet_component_manager::{FrontParameterContent, VisualSnippetComponentManager},
    },
...
pub struct SnippetManager {
    ...
    snippet_to_node_index: BiHashMap<Uuid, NodeIndex>, //mapping for snippets and pipelines
                                                       //list of uuid of snippets to index in edge adj list
...
impl SnippetManager {
    ...
    pub fn new_snippet(
        ...
        let parameters =
            external_snippet.create_parameter_components_for_parameters(sequential_id_generator);

        //call handler method, return value
        //return uuid of snippet
        return self.new_snippet_handler(
    ...
    fn new_snippet_handler(
        ...
        for pipeline_connector in pipeline_connectors.iter() {
            self.pipeline_connector_to_snippet
                .insert(pipeline_connector.get_uuid(), snippet_uuid);
        }
        ...
        for parameter in parameters.iter() {
            self.parameter_to_snippet
                .insert(parameter.uuid, snippet_uuid);
        }
    ...
    pub fn delete_snippet(&mut self, uuid: &Uuid) -> Result<(), &'static str> {
        ...
        let snippet_component = match self.find_snippet(uuid) {
            ...
            None => {
                return Err(
                    "snippet component with snippet uuid does not exist in snippet manager",
                );
            }
        };
        ...
        for pipeline_connector_uuid in pipeline_connectors_uuid.into_iter() {
            match self
                .pipeline_connector_to_snippet
                .remove(&pipeline_connector_uuid)
            {
                Some(_) => (),
                None => {
    ...
    pub fn get_snippet_graph(
        &self,
    ) -> petgraph::stable_graph::StableGraph<Uuid, (), petgraph::Directed> {
        let mut new_graph =
            petgraph::stable_graph::StableGraph::<Uuid, (), petgraph::Directed>::new();
        ...
        for node_index in self.snippet_graph.node_indices() {
            ...
            let snippet_uuid = self
                .snippet_to_node_index
        ...
        for edge_index in self.snippet_graph.edge_indices() {
            ...
            let (node_left_index, node_right_index) =
                self.snippet_graph.edge_endpoints(edge_index).unwrap();
    ...
    pub fn create_pipeline(
...

        {
            ...
            let from_snippet = match self.find_snippet(&from_snippet_uuid) {
                Some(result) => result,
                None => return Err("from snippet from pipeline connector not found"),
            };
            ...
            let to_snippet = match self.find_snippet(&to_snippet_uuid) {
                Some(result) => result,
                None => return Err("to snippet from pipeline connector not found"),
            };
...

        {
            ...
            let edge_result = self
                .snippet_graph
                .find_edge(from_snippet.graph_uuid, to_snippet.graph_uuid);
            ...
            graph_uuid_container = Some(match edge_result {
                ...
                None => {
                    ...
                    let edge_index = self.snippet_graph.add_edge(
                        from_snippet.graph_uuid,
                        to_snippet.graph_uuid,
                        1,
                    );
        ...
        //create new pipeline
        let pipeline_component = PipelineComponent::new(
            graph_uuid_container.unwrap(),
            sequential_id_generator,
            &from_uuid,
            &to_uuid,
        );
        ...
        match self.pipeline_connector_to_pipeline.get_mut(&from_uuid) {
            ...
            None => {
                let uuids = HashSet::<Uuid>::from([pipeline_uuid]);
    ...
    pub fn delete_pipeline(&mut self, uuid: &Uuid) -> Result<(), &'static str> {
        ...
        let edge_weight = self
            .snippet_graph
            .edge_weight_mut(graph_uuid_container.unwrap())
            .unwrap();
        ...
        // delete the edge
            ...
            self.snippet_graph
                .remove_edge(graph_uuid_container.unwrap());
        }
        ...
        match self
            .pipeline_connector_to_pipeline
            .get_mut(&from_pipeline_connector_uuid)
        {
            Some(uuids) => {
        ...
        if delete_entry {
            self.pipeline_connector_to_pipeline
                .remove(&from_pipeline_connector_uuid);
        }
        ...
        match self
            .pipeline_connector_to_pipeline
            .get_mut(&to_pipeline_connector_uuid)
        {
            Some(uuids) => {
                // if uuids contains the pipeline connector, remove it
                let contained = uuids.remove(&uuid);
        ...
        if delete_entry {
            self.pipeline_connector_to_pipeline
                .remove(&to_pipeline_connector_uuid);
        }
    ...
    pub fn validate_pipeline(
...

        {
            ...
            let from_result = self
                .find_pipeline_uuids_from_pipeline_connector(&from_uuid)
                .len()
                > 0;

            let to_result = self
                .find_pipeline_uuids_from_pipeline_connector(&to_uuid)
                .len()
                > 0;
        ...
        let dag_valid = match edge_result {
            ...
            None => {
                ...
                let edge_index =
                    self.snippet_graph
                        .add_edge(from_snippet.graph_uuid, to_snippet.graph_uuid, 1);
    ...
    pub fn check_pipeline_connector_capacity_full(&self, pipeline_connector_uuid: &Uuid) -> bool {
        ...
        // TODO allow multiple
        return self
            .find_pipeline_uuids_from_pipeline_connector(pipeline_connector_uuid)
            .len()
            > 0;
    }
    ...
    pub fn generate_snippet_io_point_mappings(
        &self,
    ) -> HashMap<(Uuid, String), Vec<(Uuid, String)>> {
        ...
        for (_pipeline_component_uuid, pipeline_component) in self.pipelines.iter() {
            ...
            let from_pipeline_connector_uuid =
                pipeline_component.from_pipeline_connector_uuid.clone();

            // get snippet
            // can unwrap safely here, if it fails, there is a critical code logic error
            let snippet_component_uuid = self
                .pipeline_connector_to_snippet
                .get(&from_pipeline_connector_uuid)
                .unwrap();
            ...
            for pipeline_connector in snippet_component.pipeline_connectors.iter() {
                if pipeline_connector
                    .get_uuid()
                    .eq(&from_pipeline_connector_uuid)
                {
            ...
            let snippet_component_uuid = self
                .pipeline_connector_to_snippet
                .get(&to_pipeline_connector_uuid)
                .unwrap();
            let snippet_component = self.snippets.get(snippet_component_uuid).unwrap();
            ...
            match map.get_mut(&(from_snippet_uuid.to_owned(), from_name.to_owned().unwrap())) {
                ...
                None => {
                    ...
                    map.insert(
                        (from_snippet_uuid, from_name.unwrap()),
                        Vec::from([(to_snippet_uuid, to_name.unwrap())]),
                    );
                }
            }
        }
...
impl SnippetComponent {
    ...
    /// # Arguments
    /// * 'uuid' - uuid of the pipeline connector
    pub fn find_pipeline_connector(&self, uuid: Uuid) -> Option<&PipelineConnectorComponent> {
        //find pipeline in vector
        return self
            .pipeline_connectors
            .iter()
            .find(|pipe: &&PipelineConnectorComponent| pipe.uuid == uuid);
    }
    ...
    /// # Arguments
    /// * 'uuid' - uuid of the pipeline connector
    pub fn find_pipeline_connector_mut(
        &mut self,
        uuid: Uuid,
    ) -> Option<&mut PipelineConnectorComponent> {
        //find pipeline in vector
        return self
            .pipeline_connectors
            .iter_mut()
    ...
    pub fn get_snippet_to_front_snippet(
        &self,
        ...
        let front_pipeline_connectors = self.get_pipeline_connectors_as_pipeline_connector_content(
            visual_snippet_component_manager,
            sequential_id_generator,
        );

        //get parameters as front
        let front_parameters = self.get_parameters_as_parameter_content(
            visual_snippet_component_manager,
            sequential_id_generator,
        );
    ...
    fn get_pipeline_connectors_as_pipeline_connector_content(
        &self,
        visual_snippet_component_manager: &mut VisualSnippetComponentManager,
        sequential_id_generator: &mut SequentialIdGenerator,
    ) -> Vec<FrontPipelineConnectorContent> {
        //generate contents vector
        let mut contents: Vec<FrontPipelineConnectorContent> =
            Vec::with_capacity(self.pipeline_connectors.len());

        // have this approach for many
        //get io points
        for pipeline_connector in self.pipeline_connectors.iter() {
            //push to contents
            contents.push(FrontPipelineConnectorContent::new(
                visual_snippet_component_manager,
                sequential_id_generator.get_id(),
    ...
    fn get_parameters_as_parameter_content(
        &self,
        visual_snippet_component_manager: &mut VisualSnippetComponentManager,
        sequential_id_generator: &mut SequentialIdGenerator,
    ) -> Vec<FrontParameterContent> {
        ...
        for parameter in self.parameters.iter() {
            ...
            contents.push(FrontParameterContent::new(
                visual_snippet_component_manager,
                sequential_id_generator.get_id(),
                parameter.uuid,
    ...
    pub fn get_pipeline_connector_uuids(&self) -> Vec<Uuid> {
        return (&self.pipeline_connectors)
            .into_iter()
            .map(|pcc| -> Uuid {
                return pcc.uuid;
            })
    ...
    pub fn get_input_names(&self) -> Vec<String> {
        return self
            .pipeline_connectors
            .iter()
            .filter(|connector| -> bool { connector.input == true })
...
impl PipelineComponent {
    ...
    pub fn create_pipeline_as_front_content(
        &self,
        visual_snippet_component_manager: &mut VisualSnippetComponentManager,
        sequential_id_generator: &mut SequentialIdGenerator,
    ) -> FrontPipelineContent {
        let front_pipeline_content = FrontPipelineContent::new(
            visual_snippet_component_manager,
            sequential_id_generator.get_id(),
            self.uuid,
        );
...
impl SnippetParameterComponent {
    pub fn new(
        storage: SnippetParameterBaseStorage,
        name: String,
        p_type: ExternalSnippetParameterType,
        sequential_id_generator: &mut SequentialIdGenerator,
    ) -> Self {
...
mod tests {
    ...
    fn test_new_snippet() {
        ...
        pipeline_connectors.push(PipelineConnectorComponent::new(
            ...
            external_pipeline_connector_uuid,
            "input_pipeline_connector_one",
            true,
        ));

        let parameter_storage = ExternalSnippetParameterType::into_storage_type(
            &ExternalSnippetParameterType::SingleLineText,
        );
        let parameter = SnippetParameterComponent::new(
            parameter_storage,
            "param_one".to_string(),
            ExternalSnippetParameterType::SingleLineText,
            &mut sequential_id_generator,
        ...
        let snippet_id = snippet_manager.new_snippet_handler(
            &mut sequential_id_generator,
            pipeline_connectors,
            parameters,
        ...
        let snippet_node_weight_result = snippet_manager
            .snippet_graph
            .node_weight(snippet.graph_uuid);
        ...
        assert_eq!(
            snippet.pipeline_connectors.get(0).unwrap().name,
            "input_pipeline_connector_one"
        );
        ...
        assert_eq!(
            snippet.parameters.get(0).unwrap().p_type,
            ExternalSnippetParameterType::SingleLineText
        );
    ...
    fn delete_snippet() {
        ...
        let mut sequential_id_generator = SequentialIdGenerator::default();
        let mut pipeline_connectors = Vec::<PipelineConnectorComponent>::new();
        let parameters = Vec::<SnippetParameterComponent>::new();
        ...
        let first_snippet_uuid = snippet_manager.new_snippet_handler(
            &mut sequential_id_generator,
            pipeline_connectors,
            parameters,
        ...
        let second_snippet_uuid = snippet_manager.new_snippet_handler(
            &mut sequential_id_generator,
            pipeline_connectors,
            parameters,
        ...
        let third_snippet_uuid = snippet_manager.new_snippet_handler(
            &mut sequential_id_generator,
            pipeline_connectors,
            parameters,
    ...
    fn test_create_pipeline() {
        ...
        snippet_manager
            .create_pipeline(&mut sequential_id_generator, 2, 12)
            .unwrap();

        // connect three to one
        snippet_manager
            .create_pipeline(&mut sequential_id_generator, 7, 13)
            .unwrap();
        ...
        assert!(match snippet_manager
            .snippet_graph
            .find_edge(snippet_one_graph_uuid, snippet_three_graph_uuid)
        {
            Some(_) => true,
            None => false,
        });
        assert!(match snippet_manager
            .snippet_graph
            .find_edge(snippet_two_graph_uuid, snippet_three_graph_uuid)
        {
            Some(_) => true,
            None => false,
        });

        let edge_one = snippet_manager
            .snippet_graph
            .find_edge(snippet_one_graph_uuid, snippet_three_graph_uuid)
            .unwrap();
        let edge_two = snippet_manager
            .snippet_graph
            .find_edge(snippet_two_graph_uuid, snippet_three_graph_uuid)
            .unwrap();
